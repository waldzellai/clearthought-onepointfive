...
**Model enhancement servers** are like pen and paper, or a calculator: general-purpose cognitive tools natively designed for LLM use. They extend the model's abilities across a variety of circumstances, not just specific API integrations.
...
### The Three Agentic Capabilities

Model enhancement servers can provide meaningful levels of all three canonical capabilities that elevate an "LLM" to an "agent":

1. **Retrieval**: Looking up and accessing stored information
2. **Tool use**: Executing operations and processing data
3. **Memory**: Maintaining state and context across interactions

These capabilities often overlap and reinforce each other within a single server. **Model enhancement via MCP is often a low-overhead way to extend agent capabilities**, particularly for structured reasoning processes.
...
3. **Making processes transparent**: We can see and analyze what the agent did
