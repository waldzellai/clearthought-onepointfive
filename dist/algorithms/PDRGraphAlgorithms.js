...
export class PDRGraphAlgorithms {
     ...
     */
    detectConnectedComponents(graph) {
     ...
     */
    calculateCoherence(graph, nodeIds) {
        if (nodeIds.size <= 1)
            return 1;
        let internalEdges = 0;
    ...
    findMissingConnections(graph, gaps) {
        const nodes = graph.getAllNodes();
        ...
        nodes.forEach((node1) => {
            nodes.forEach((node2) => {
                if (node1.id >= node2.id)
                    return;
                // Check tag overlap
                const commonTags = Array.from(node1.metadata.tags).filter((tag) => node2.metadata.tags.has(tag));
    ...
    findLowConfidenceAreas(graph, gaps) {
        const nodes = graph.getAllNodes();
        nodes.forEach((node) => {
    ...
    findIsolatedClusters(graph, gaps) {
     ...
     */
    selectTopNodes(graph, centrality, criteria) {
