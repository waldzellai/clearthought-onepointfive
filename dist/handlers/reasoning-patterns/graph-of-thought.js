...
export class GraphOfThoughtHandler {
	...
	addNode(nodeData, session) {
		...
		session.stats.nodesByType[node.nodeType] = (session.stats.nodesByType[node.nodeType] || 0) + 1;
	...
	connectNodes(sourceId, targetId, edgeType, weight, session) {
		...
		session.stats.edgesByType[edgeType] = (session.stats.edgesByType[edgeType] || 0) + 1;
		this.updateGraphStats(session);
		// Auto-prune weak edges if enabled
		if (
			session.config.autoPruneWeakEdges &&
			session.config.edgeWeightThreshold &&
			weight < session.config.edgeWeightThreshold
		) {
			this.removeEdge(edgeId, session);
			throw new Error(
				`Edge weight ${weight} below threshold ${session.config.edgeWeightThreshold}`,
			);
		}
		return edge;
	}
	...
	calculateClusterCohesion(nodeIds, session) {
		...
		let internalEdges = 0;
		const possibleEdges = nodeIds.length * (nodeIds.length - 1);
