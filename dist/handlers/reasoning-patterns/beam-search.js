...
export class BeamSearchHandler {
	...
	calculatePathScore(path, session) {
		...
		switch (session.config.scoreAggregation) {
			...
			case "weighted": {
				...
				path.nodeIds.forEach((nodeId, index) => {
					const node = session.nodes.get(nodeId);
					if (node) {
						const weight = 0.9 ** (path.nodeIds.length - index - 1);
		...
		components.finalScore =
			components.baseScore + (components.diversityBonus || 0) + (components.lengthAdjustment || 0);
	...
	mergeNodeSequences(sequences) {
		...
		for (let i = 0; i < maxLength; i++) {
			const nodesAtPosition = sequences.filter((seq) => seq.length > i).map((seq) => seq[i]);
