...
export class MCTSHandler {
	 ...
	 */
	isLeaf(node) {
		return (node.untriedActions && node.untriedActions.length > 0) || node.childrenIds.length === 0;
	}
	/**
	...
	calculateUCB(nodeId, parentVisits, session) {
		...
		switch (session.config.ucbVariant) {
			case "ucb1":
				...
				components.explorationTerm =
					session.explorationConstant * Math.sqrt(Math.log(parentVisits) / node.visits);
				break;
			case "ucb1-tuned": {
				components.exploitationTerm = node.averageValue;
				const variance = this.calculateVariance(node);
				const V = variance + Math.sqrt((2 * Math.log(parentVisits)) / node.visits);
				components.explorationTerm = Math.sqrt(
					(Math.log(parentVisits) / node.visits) * Math.min(0.25, V),
				);
				break;
			}
			case "puct": {
				...
				components.exploitationTerm = node.averageValue;
				const prior =
					node.metadata?.priorProbability ||
					1 / (node.parentId ? session.nodes.get(node.parentId)?.childrenIds.length || 1 : 1);
				components.priorBias = prior;
				components.explorationTerm =
					(session.explorationConstant * prior * Math.sqrt(parentVisits)) / (1 + node.visits);
				break;
			}
			default:
				components.exploitationTerm = node.averageValue;
		...
		if (session.config.useRAVE && node.metadata?.action) {
			...
			if (raveStats.visits > 0) {
				const beta = Math.sqrt(
					session.config.raveBias / (3 * node.visits + session.config.raveBias),
				);
				components.raveTerm = beta * raveStats.averageValue;
				components.exploitationTerm =
					(1 - beta) * components.exploitationTerm + beta * raveStats.averageValue;
			}
		}
		components.ucbScore =
			components.exploitationTerm + components.explorationTerm + (components.priorBias || 0);
	...
	isTerminalState(action, session) {
		...
		return action === "terminate" || session.stats.maxDepth >= session.simulationDepth;
	}
	/**
	...
	neuralRollout(startNode, session, result) {
		...
		return (
			this.heuristicRollout(startNode, session, result) * 0.7 +
	...
	checkTermination(session) {
		...
		if (
			criteria.minVisitsThreshold &&
			session.stats.bestActionVisits &&
			session.stats.bestActionVisits >= criteria.minVisitsThreshold
		) {
			return true;
		}
		return false;
	...
	updateStatistics(session, iterationTime) {
		...
		if (rootNode) {
			...
			session.stats.explorationRatio =
				rootNode.childrenIds.length > 0 ? exploredChildren / rootNode.childrenIds.length : 0;
		}
	}
	/**
			...
			if (child && child.metadata?.action) {
				stats.push({
					...
					winRate: child.metadata.outcomes
						? child.metadata.outcomes.wins / child.visits
						: undefined,
