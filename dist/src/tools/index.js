...
export function registerTools(server, sessionState) {
	server.tool(
		...
		async (args) => handleClearThoughtTool(sessionState, args),
	);
}
/**
...
export async function executeClearThoughtOperation(sessionState, operation, args) {
	...
	switch (operation) {
		...
		case "collaborative_reasoning": {
			...
			const contributions = parameters.contributions || [];
		...
		case "research": {
			...
			const findings = parameters.findings || [];
		...
		case "analogical_reasoning": {
			...
			const inferredInsights = parameters.inferredInsights || [];
		...
		case "statistical_reasoning": {
			...
			if (mode === "summary") {
				...
				const normalizedPrior = Object.fromEntries(
		...
		case "ethical_analysis": {
			...
			const findings = parameters.findings || [];
			const risks = parameters.risks || [];
			const mitigations = parameters.mitigations || [];
