...
export class UnifiedStore {
	...
	exportByType(type) {
		const entries = type
			? Array.from(this.typeIndexes.get(type) || [])
					...
					.filter(Boolean)
			: this.getAll();
		const exportData = entries.length === 1 ? entries[0].data : entries.map((e) => e.data);
		return {
	...
	importData(sessionExport) {
		const dataArray = Array.isArray(sessionExport.data) ? sessionExport.data : [sessionExport.data];
