...
const GoTSchema = z.object({
	...
	config: z
		.object({
			...
			analysisAlgorithms: z
				.array(z.enum(["centrality", "clustering", "paths", "cycles"]))
				.optional(),
			defaultNodeType: z
				.enum([
					"hypothesis",
...
async function handleGoT(args, _session) {
	...
	const analysis = session.analysisMetrics
		? {
				centralityNodes: session.analysisMetrics.centrality
					? session.analysisMetrics.centrality.size
					: 0,
