...
export class DecisionFrameworkOperation extends BaseOperation {
	...
	async execute(context) {
		...
		const generatedAlternatives =
			alternatives.length === 0 ? this.generateAlternatives(prompt) : alternatives;
	...
	generateAlternatives(decision) {
		...
		if (
			decision.toLowerCase().includes("technology") ||
	...
	calculateConfidence(analysis) {
		...
		const scores = evaluations.map(
			(e) => e.score || e.roi || e.overallSupport || e.weightedTotal || 5,
		);
