...
export class DecisionFrameworkOperation extends BaseOperation {
    ...
    async execute(context) {
        ...
        const stakeholders = parameters.stakeholders || [];
        // Generate alternatives if not provided
        const generatedAlternatives = alternatives.length === 0 ? this.generateAlternatives(prompt) : alternatives;
        // Generate criteria if not provided
        const generatedCriteria = criteria.length === 0 ? this.generateCriteria(framework) : criteria;
        // Apply the selected framework
        const analysis = this.applyFramework(framework, generatedAlternatives, generatedCriteria, weights, stakeholders);
        const decisionData = {
            decision: prompt,
            framework,
    ...
    generateAlternatives(decision) {
        ...
        if (decision.toLowerCase().includes("technology") ||
            decision.toLowerCase().includes("software")) {
    ...
    generateCriteria(framework) {
        ...
        const criteriaNames = baseMetrics[framework] || baseMetrics.pros_cons;
    ...
    applyFramework(framework, alternatives, criteria, weights, stakeholders) {
        switch (framework) {
            case "pros_cons":
    ...
    applyProsConsFramework(alternatives, criteria) {
        return {
            type: "pros_cons",
    ...
    applyCostBenefitFramework(alternatives, criteria) {
        return {
            type: "cost_benefit",
            evaluations: alternatives.map((alt) => ({
                alternative: alt,
    ...
    applyRiskAssessmentFramework(alternatives, criteria) {
        return {
    ...
    applyStakeholderFramework(alternatives, criteria, stakeholders) {
        return {
            type: "stakeholder",
            evaluations: alternatives.map((alt) => ({
                alternative: alt,
    ...
    applyGenericFramework(alternatives, criteria, weights) {
        return {
    ...
    generateRecommendation(analysis) {
        ...
        evaluations.forEach((evaluation) => {
            ...
            else if (evaluation.roi)
                score = evaluation.roi / 50; // Normalize ROI
            else if (evaluation.overallSupport)
                score = evaluation.overallSupport;
            else if (evaluation.weightedTotal)
    ...
    calculateConfidence(analysis) {
        ...
        const scores = evaluations.map((e) => e.score || e.roi || e.overallSupport || e.weightedTotal || 5);
