...
export class SocraticMethodOperation extends BaseOperation {
    ...
    async execute(context) {
        ...
        const previousQuestions = parameters.previousQuestions || [];
        const responses = parameters.responses || [];
        // Generate question sequence based on type and depth
        const questionSequence = this.generateQuestionSequence(prompt, questionType, depth, previousQuestions);
    ...
    generateQuestionSequence(topic, questionType, depth, previousQuestions) {
        const questionTypes = {
        ...
        const generator = questionTypes[questionType] || questionTypes.clarification;
        const newQuestions = generator.call(this, topic, depth);
        return [...previousQuestions, ...newQuestions.slice(0, depth)];
    }
    generateClarificationQuestions(topic, depth) {
        return [
            {
                type: "clarification",
    ...
    generateAssumptionQuestions(topic, depth) {
        return [
            {
                type: "assumptions",
    ...
    generateEvidenceQuestions(topic, depth) {
        return [
            {
                type: "evidence",
    ...
    generatePerspectiveQuestions(topic, depth) {
        return [
            {
                type: "perspective",
    ...
    generateImplicationQuestions(topic, depth) {
        return [
            {
                type: "implications",
    ...
    generateMetaQuestions(topic, depth) {
        return [
            {
                type: "meta",
    ...
    analyzeAssumptions(assumptions) {
    ...
    categorizeAssumption(assumption) {
        ...
        if (assumption.toLowerCase().includes("cause") ||
            assumption.toLowerCase().includes("because")) {
            return "causal";
        }
        return "descriptive";
    }
    buildDialogueTree(questions, responses) {
        const tree = {
    ...
    extractInsights(questions, responses) {
        const insights = [];
    ...
    generateNextQuestion(questions, responses) {
        ...
        return {
            question: followUpTemplates[lastQuestion.type] ||
    ...
    isExplorationComplete(questions, responses, targetDepth) {
        return questions.length >= targetDepth && responses.length >= Math.floor(targetDepth * 0.8); // 80% response rate
    }
}
