...
export class TreeOfThoughtOperation extends BaseOperation {
    ...
    async execute(context) {
        ...
        sessionState.addOperationEntry("tree_of_thought", state.currentPhase, state);
     ...
     */
    getOrInitializeState(sessionState, parameters) {
        ...
        if (existingState && existingState.data) {
            ...
            if (data.evaluations && !(data.evaluations instanceof Map)) {
                data.evaluations = new Map(Object.entries(data.evaluations));
            }
            return data;
        }
        // Initialize new state
        const depth = this.getParam(parameters, "depth", 3);
        const breadth = this.getParam(parameters, "breadth", 3);
        return {
            depth,
            breadth,
            currentDepth: 0,
     ...
     */
    async executePhase(context, state) {
    ...
    handleBranchGeneration(context, state) {
        ...
        if (submittedBranches.length > 0) {
            ...
            logger.logStep("tree_of_thought", state.currentDepth * 2 + 1, state.depth * 2, `Generated ${submittedBranches.length} branches at depth ${state.currentDepth}`);
            // Move to evaluation phase
            state.currentPhase = "evaluation";
            return this.handleBranchEvaluation(context, state);
        }
        // Request branch generation
        logger.logStep("tree_of_thought", state.currentDepth * 2, state.depth * 2, `Requesting ${state.breadth} branches at depth ${state.currentDepth}`);
        return {
...
Return your response as a JSON array of strings, where each string is a complete branch description.`,
    ...
    handleBranchEvaluation(context, state) {
        ...
        if (submittedEvaluations.length > 0) {
            ...
            logger.logStep("tree_of_thought", state.currentDepth * 2 + 2, state.depth * 2, `Evaluated ${submittedEvaluations.length} branches`);
            // Move to selection phase
            state.currentPhase = "selection";
            return this.handleBranchSelection(context, state);
        }
        // Request evaluations
        logger.logStep("tree_of_thought", state.currentDepth * 2 + 1, state.depth * 2, `Requesting evaluation of ${branchesAtDepth.length} branches`);
        return {
            operation: "tree_of_thought",
            progress: this.getProgress(context.sessionState),
            nextStep: {
...
Return as JSON array: [{ branchId, feasibility, completeness, innovation, reasoning }, ...]`,
     ...
     */
    handleBranchSelection(context, state) {
        const branchesAtDepth = state.branches.filter((b) => b.depth === state.currentDepth);
        ...
        const selectedBranches = this.getParam(context.parameters, "selectedBranches", []);
        ...
        // Request selection
        logger.logStep("tree_of_thought", state.currentDepth * 2 + 2, state.depth * 2, `Select top ${topCount} branches for depth ${state.currentDepth + 1}`);
        return {
            operation: "tree_of_thought",
            progress: this.getProgress(context.sessionState),
            nextStep: {
...
${topBranches
                    .map((b, i) => `${i + 1}. ${b.id}: ${b.content.substring(0, 100)}...
...
Return the IDs of branches to explore as JSON array: ["branch_id_1", "branch_id_2", ...]`,
     ...
     */
    handleCompletion(context, state) {
        // Find the best path through the tree
        const bestPath = this.findBestPath(state);
        logger.logStep("tree_of_thought", state.depth * 2, state.depth * 2, `Exploration complete. Best path: ${bestPath.map((b) => b.id).join(" → ")}`);
        return {
            operation: "tree_of_thought",
            progress: this.getProgress(context.sessionState),
            nextStep: {
    ...
    getToolDescription() {
        return {
            name: "tree_of_thought",
            description: "Systematic tree-based exploration with branching, evaluation, and selection. Guides AI through generating multiple approaches, evaluating them on feasibility/completeness/innovation, and selecting the best paths to explore further.",
            inputSchema: {
                type: "object",
                properties: {
                    prompt: {
