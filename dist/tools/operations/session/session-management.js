import { z } from 'zod';
export function registerSessionManagement(server, sessionState) {
    // Session Info Tool
    server.tool('session_info', 'Get information about the current session including statistics and recent activity', {}, async () => {
    ...
    // Session Export Tool
    server.tool('session_export', 'Export the entire session state for backup or sharing', {
    ...
    // Session Import Tool
    server.tool('session_import', 'Import a previously exported session state', {
