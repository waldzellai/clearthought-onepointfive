...
export class NotebookExportOperation extends BaseOperation {
	...
	async execute(context) {
		...
		const format = this.getParam(parameters, "format", "srcmd");
		...
		switch (format) {
			case "srcmd":
				...
				suggestedFilename =
					filename || `${this.sanitizeFilename(notebook.metadata?.title || "notebook")}.srcmd`;
				break;
			case "json":
				exportContent = this.exportToJson(notebook, includeMetadata, includeOutputs);
				mimeType = "application/json";
				suggestedFilename =
					filename || `${this.sanitizeFilename(notebook.metadata?.title || "notebook")}.json`;
				break;
			case "html":
				exportContent = this.exportToHtml(notebook, includeMetadata, includeOutputs);
				mimeType = "text/html";
				suggestedFilename =
					filename || `${this.sanitizeFilename(notebook.metadata?.title || "notebook")}.html`;
				break;
			case "markdown":
				exportContent = this.exportToMarkdown(notebook, includeOutputs);
				mimeType = "text/markdown";
				suggestedFilename =
					filename || `${this.sanitizeFilename(notebook.metadata?.title || "notebook")}.md`;
				break;
			default:
		...
		return this.createResult({
			...
			instructions: {
				...
				formats:
					"SrcMD for sourcebook, JSON for data exchange, HTML for viewing, Markdown for documentation",
	 ...
	 */
	exportToSrcMd(notebook, includeOutputs) {
		const notebookStore = NotebookCreateOperation.getNotebookStore();
	 ...
	 */
	exportToJson(notebook, includeMetadata, includeOutputs) {
		const notebookStore = NotebookCreateOperation.getNotebookStore();
		const jsonData = notebookStore.exportToJson(notebook.id);
		if (!jsonData) {
		...
		if (!includeOutputs) {
			jsonData.cells = jsonData.cells.map((cell) => {
	 ...
	 */
	exportToHtml(notebook, includeMetadata, includeOutputs) {
...
`;
	 ...
	 */
	exportToMarkdown(notebook, includeOutputs) {
		const title = notebook.metadata?.title || "Clear Thought Notebook";
		const description = notebook.metadata?.description || "";
		let markdown = `# ${title}\n\n`;
		if (description) {
...
`;
	 ...
	 * Render cell as HTML
	 */
	renderCellAsHtml(cell, index, includeOutputs) {
	 ...
	 * Generate export statistics
	 */
	generateExportStats(notebook, content) {
		return {
