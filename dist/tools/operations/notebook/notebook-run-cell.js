...
export class NotebookRunCellOperation extends BaseOperation {
	...
	async execute(context) {
		...
		if (cellId) {
			...
			if (!targetCell) {
				throw new Error(
					"No executable code cell found. Please specify cellId or add a code cell first.",
				);
			}
		}
		...
		// Execute cell
		const execution = await notebookStore.executeCell(notebook.id, targetCell.id, timeoutMs);
		...
		return this.createResult({
			...
			instructions: {
				...
				debugging: execution.error
					? "Review error message and check code syntax"
	 ...
	 */
	findTargetCell(notebook, prompt) {
	 ...
	 * Analyze execution results
	 */
	analyzeExecution(execution, cell) {
		const analysis = {
