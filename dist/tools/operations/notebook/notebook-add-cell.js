...
export class NotebookAddCellOperation extends BaseOperation {
    ...
    async execute(context) {
        ...
        const cellType = this.getParam(parameters, "type", "markdown");
        ...
        const cell = notebookStore.addCell(notebook.id, cellType, source, cellType === "code" ? language : undefined, index);
        if (!cell) {
        ...
        return this.createResult({
            ...
            instructions: {
                execution: cellType === "code"
                    ? "Use notebook-run-cell to execute this code cell"
                    : "Cell added as documentation",
     ...
     */
    generateCellMetadata(cellType, source, language) {
        const metadata = {
            type: cellType,
     ...
     */
    detectContentType(source, cellType) {
        if (cellType === "code") {
