...
export class NotebookCreateOperation extends BaseOperation {
    ...
    category = "notebook";
    static notebookStore = new EphemeralNotebookStore();
     ...
     */
    addInitialCell(notebookId, content) {
        const type = content.type || "markdown";
        const source = content.source || content.content || "";
        const language = content.language || "javascript";
        NotebookCreateOperation.notebookStore.addCell(notebookId, type, source, type === "code" ? language : undefined);
    }
    /**
     * Add default introduction cell
     */
    addDefaultIntroduction(notebookId, title, description) {
...
`;
        NotebookCreateOperation.notebookStore.addCell(notebookId, "markdown", introContent);
    }
    /**
