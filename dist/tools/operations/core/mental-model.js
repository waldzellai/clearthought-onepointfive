...
export class MentalModelOperation extends BaseOperation {
    ...
    validateData(input) {
        ...
        if (data.modelName && !validModels.includes(data.modelName)) {
            throw new Error(`Invalid modelName: must be one of ${validModels.join(", ")} (got: ${data.modelName})`);
        }
        return {
            entry: data.entry,
     ...
     */
    formatEntry(data) {
        const { entryNumber, totalEntries, entry, isRevision, revisesEntry, branchFromEntry, branchId, modelName } = data;
        let prefix = "";
        let context = "";
        if (isRevision) {
