...
8. Only set nextEntryNeeded to false when metacognitive monitoring is complete`,
     ...
     */
    formatEntry(data) {
        const { entryNumber, totalEntries, entry, isRevision, revisesEntry, branchFromEntry, branchId, confidence, } = data;
        let prefix = "";
        let context = "";
        if (isRevision) {
    ...
    async execute(context) {
        try {
            ...
            if (validatedInput.confidence !== undefined) {
                context.sessionState.updateKPI("metacognitive_confidence", validatedInput.confidence, "Thinking Confidence", 0.8, "up");
            }
            // Terminal logging
