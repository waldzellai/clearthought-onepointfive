...
export class PDRReasoningOperation extends BaseOperation {
	...
	async execute(context) {
		...
		const action = this.getParam(context.parameters, "action", "continue");
		...
		switch (action) {
			case "start": {
				...
				const globalSelection = this.getParam(context.parameters, "globalSelection", {});
				session = {
					id: sessionId,
					subjects: new Map(),
					passes: [],
			...
			case "add_subject": {
				...
				const subjectTags = this.getParam(context.parameters, "tags", []);
				const subject = {
			...
			case "run_pass": {
				...
				const passName = this.getParam(context.parameters, "passName", "scan");
				const approach = this.getParam(context.parameters, "approach", null);
			...
			case "select": {
				...
				const criteria = this.getParam(context.parameters, "criteria", session.globalSelection);
				...
				if (criteria.minScore) {
					filtered = filtered.filter((s) => {
						const avgScore =
							Object.values(s.passScores).reduce((a, b) => a + b, 0) /
							Object.values(s.passScores).length;
				...
				if (criteria.topK) {
					filtered = filtered
						.sort((a, b) => {
							const avgA =
								Object.values(a.passScores).reduce((sum, score) => sum + score, 0) /
								Object.values(a.passScores).length;
							const avgB =
								Object.values(b.passScores).reduce((sum, score) => sum + score, 0) /
								Object.values(b.passScores).length;
							return avgB - avgA;
						})
			...
			case "export": {
				...
				const exportData = {
					...
					subjects: Array.from(session.subjects.entries()).map(([id, subject]) => ({
						...subject,
						id,
					})),
		...
		return this.createResult({
			...
			availablePasses: session.passes.map((p) => p.name),
			nextPass:
				completedPasses < session.passes.length ? session.passes[completedPasses].name : null,
			recentTrace:
				session.traces.length > 0
					? {
							passName: session.passes.find(
								(p) => p.id === session.traces[session.traces.length - 1].policyId,
							)?.name,
							processedSubjects:
								session.traces[session.traces.length - 1].processedSubjectIds.length,
