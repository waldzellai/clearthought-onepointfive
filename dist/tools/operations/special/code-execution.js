...
export class CodeExecutionOperation extends BaseOperation {
	...
	async execute(context) {
		...
		const language = this.getParam(context.parameters, "language", "python");
	 ...
	 */
	performSafetyCheck(code, language) {
		const issues = [];
		...
		const dangerousPatterns = [
			{
				...
				message: "OS module usage detected - potential system access",
			},
			{
				pattern: /subprocess|popen|system/,
				message: "Subprocess execution detected - potential command injection",
			},
			{
			...
			{
				pattern: /open\s*\(.*['"](\/|\\\\|\.\.)/g,
				message: "File access outside current directory detected",
			},
			{
				pattern: /socket|urllib|requests\.get|requests\.post/i,
				message: "Network access detected - potential data exfiltration",
			},
			{
				pattern: /\bfile\s*=|mode\s*=\s*['"]w/,
				message: "File writing detected - potential data modification",
			},
			{ pattern: /rm\s+|del\s+|unlink|remove\(/i, message: "File deletion operations detected" },
			{
				pattern: /while\s+true|for.*range\(.*,.*,-1\)|while\s+1/,
		...
		if (language === "python") {
			dangerousPatterns.push(
				{
					pattern: /__import__|getattr|setattr|delattr/,
					message: "Dynamic attribute access detected",
				},
				{
					pattern: /pickle\.loads|pickle\.load/,
					message: "Pickle deserialization detected - potential code execution",
				},
	 ...
	 */
	analyzeExecutionResult(result, code) {
		const analysis = {
			performance: {
				executionTime: result.durationMs,
				performanceCategory:
					result.durationMs < 100 ? "fast" : result.durationMs < 1000 ? "moderate" : "slow",
			},
			output: {
		...
		if (result.stderr) {
			const errorPatterns = [
				...
				{ pattern: /KeyError/i, message: "Key error - accessing invalid dictionary key" },
				{
					...
					message: "Import error - missing module or package",
				},
				{ pattern: /MemoryError/i, message: "Memory error - insufficient memory available" },
				{
					pattern: /RecursionError/i,
					message: "Recursion error - maximum recursion depth exceeded",
				},
			];
