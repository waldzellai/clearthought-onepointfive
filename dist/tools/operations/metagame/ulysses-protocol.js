 ...
 */
import { advancePhase, checkConstraints, createUlyssesNode, createUlyssesSession, exportToMarkdown, makeFinalDecision, suggestNextActions, } from "../../../types/reasoning-patterns/ulysses-protocol.js";
    ...
    async execute(context) {
        ...
        switch (action) {
            ...
            case "continue": {
                ...
                if (constraintCheck.violated && constraintCheck.escalation) {
                    ...
                    const escalatedNode = createUlyssesNode(`AUTO-ESCALATION: ${constraintCheck.escalation.reason}`, session.currentPhase, 0.0);
                ...
                const scopeChange = this.getParam(context.parameters, "scopeChange", null);
                if (scopeChange) {
                    node.scopeChange = scopeChange;
                    if (!scopeChange.approved) {
                        session.metrics.scopeDrift +=
                            scopeChange.impact === "minor" ? 0.2 : scopeChange.impact === "moderate" ? 0.5 : 1.0;
                    }
                }
                // Track time spent
            ...
            case "advance": {
                ...
                if (advancement.success) {
                    ...
                    return this.createResult({
                        ...
                        requiredEvidence: session?.gates.find((g) => g.phase === session?.currentPhase)
                            ?.exitCriteria,
                    });
                }
                break;
        ...
        return this.createResult({
            ...
            currentGate: currentGate
                ...
                : undefined,
            constraintViolations: currentConstraintCheck.violated
                ? currentConstraintCheck.violations
                : undefined,
            escalation: currentConstraintCheck.escalation,
            finalDecision: session.finalDecision,
            suggestions,
            nextStepNeeded: !session.finalDecision,
            recommendedAction: session.finalDecision
                ? "complete"
                : currentConstraintCheck.violated
                    ? "address_violations"
                    : "continue",
        });
    }
