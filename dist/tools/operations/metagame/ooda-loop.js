 ...
 */
import { advancePhase, createOODANode, createOODASession, evaluateEvidenceQuality, exportToMarkdown, suggestNextActions, } from "../../../types/reasoning-patterns/ooda-loop.js";
...
export class OODALoopOperation extends BaseOperation {
    ...
    async execute(context) {
        ...
        const phase = this.getParam(context.parameters, "phase", null);
        const evidence = this.getParam(context.parameters, "evidence", []);
        ...
        switch (action) {
            ...
            case "continue": {
                ...
                const hypotheses = this.getParam(context.parameters, "hypotheses", []);
                ...
                if (nodePhase === "decide") {
                    const options = this.getParam(context.parameters, "options", []);
                    const selected = this.getParam(context.parameters, "selected", "");
                    const rationale = this.getParam(context.parameters, "rationale", "");
                ...
                if (nodePhase === "act") {
                    const actionDescription = this.getParam(context.parameters, "actionDescription", "");
                    const outcome = this.getParam(context.parameters, "outcome", "");
                    const metrics = this.getParam(context.parameters, "metrics", {});
                ...
                if (autoAdvance && evidence.length >= session.config.minEvidence) {
                    ...
                    const checklistMet = checklist.every((item) => evidence.some((e) => e.toLowerCase().includes(item.replace("_", " "))));
                    if (checklistMet) {
        ...
        return this.createResult({
            ...
            suggestions,
            nextStepNeeded: true,
            recommendedNextPhase: session.currentPhase === "act"
                ? "observe"
                : session.currentPhase === "observe"
                    ? "orient"
                    : session.currentPhase === "orient"
                        ? "decide"
                        : "act",
        });
    }
