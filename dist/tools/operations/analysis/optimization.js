...
export class OptimizationOperation extends BaseOperation {
    ...
    geneticAlgorithmOptimization(variables, objective, bounds, populationSize, maxIterations) {
        ...
        for (let generation = 0; generation < maxIterations; generation++) {
            // Evaluate fitness
            const fitness = population.map((individual) => this.evaluateObjectiveFunction(individual, objective));
        ...
        // Find best solution
        const finalFitness = population.map((individual) => this.evaluateObjectiveFunction(individual, objective));
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            for (const particle of particles) {
                ...
                if (isBetterPersonal) {
                    ...
                    particle.bestFitness = fitness;
                    // Update global best
                    const isBetterGlobal = objective === "maximize" ? fitness > globalBestFitness : fitness < globalBestFitness;
                    if (isBetterGlobal) {
                        globalBestPosition = [...particle.position];
                        globalBestFitness = fitness;
                    }
