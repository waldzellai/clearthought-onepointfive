...
export class SimulationOperation extends BaseOperation {
    ...
    async execute(context) {
        ...
        const initialState = parameters.initialState || this.parseInitialState(prompt);
        const rules = parameters.rules || this.extractRules(prompt);
    ...
    runSystemDynamicsSimulation(initialState, rules, steps, timeStep) {
        const trajectory = [];
    ...
    applySystemDynamicsRule(rule, state, timeStep) {
    ...
    runAgentBasedSimulation(agentCount, rules, steps) {
        const trajectory = [];
        // Initialize agents
    ...
    runMonteCarloSimulation(initialState, rules, iterations) {
        const results = [];
        ...
        for (const key of keys) {
            ...
            finalState[`${key}_std`] = Math.sqrt(values.reduce((sum, v) => sum + (v - finalState[key]) ** 2, 0) / values.length);
        }
        return {
            steps: iterations,
            trajectory: results,
    ...
    runDiscreteEventSimulation(initialState, events, maxTime) {
        const trajectory = [];
    ...
    runCellularAutomataSimulation(gridSize, rules, steps) {
        const trajectory = [];
        // Initialize grid
        let grid = Array.from({ length: gridSize }, () => Array.from({ length: gridSize }, () => (Math.random() > 0.5 ? 1 : 0)));
        for (let step = 0; step < steps; step++) {
            ...
            grid = newGrid;
            // Record statistics
            const aliveCells = grid.flat().reduce((sum, cell) => sum + cell, 0);
            trajectory.push({
                time: step,
        ...
        const finalAliveCells = grid.flat().reduce((sum, cell) => sum + cell, 0);
        const finalState = {
    ...
    generateInsights(result, simulationType) {
        const insights = [];
        if (result.trajectory.length === 0) {
    ...
    generateRecommendations(result, simulationType) {
