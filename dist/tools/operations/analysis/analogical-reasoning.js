...
export class AnalogicalReasoningOperation extends BaseOperation {
	...
	validateData(input) {
		...
		for (const mapping of analogy.mappings) {
			...
			if (
				mapping.mappingType &&
				!["role", "structure", "behavior", "constraint"].includes(mapping.mappingType)
			) {
				throw new Error(
		...
		return {
			...
			refinesEntry: data.refinesEntry,
		};
	}
	/**
...
└${border}┘`;
	 ...
	 */
	generateNextSteps(data, allMappings) {
		const steps = [];
		const { mappings } = data.analogy;
		...
		const allTypes = ["role", "structure", "behavior", "constraint"];
