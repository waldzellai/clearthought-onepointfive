...
export class CustomFrameworkOperation extends BaseOperation {
	 ...
	 */
	generateDefaultSteps(prompt, domain) {
		const baseSteps = [
	 ...
	 */
	generateDefaultPrinciples(domain) {
	 ...
	 */
	generateDefaultConstraints(domain) {
		return [
	 ...
	 */
	generateQualityGates(principles) {
	 ...
	 */
	generateValidationCriteria(outputs) {
	 ...
	 */
	generateFrameworkDocumentation(framework) {
		return `
...
### Guiding Principles
${framework.structure.principles.map((principle) => `- ${principle}`).join("\n")}

### Constraints
${framework.structure.constraints.map((constraint) => `- ${constraint}`).join("\n")}

### Expected Outputs
${framework.structure.expectedOutputs.map((output) => `- ${output}`).join("\n")}

## Implementation Guide

### Quality Checkpoints
${framework.implementation.checkpoints
	.map(
		(cp) => `
**Phase ${cp.checkpoint}: ${cp.phase}**
${cp.criteria.map((criterion) => `- [ ] ${criterion}`).join("\n")}
`,
	)
	.join("")}
...
4. Iterate as needed for improvement
	 ...
	 */
	generateApplicationGuidance(framework) {
		return {
			stepByStep: framework.structure.phases.map((phase) => ({
				phase: phase.name,
				actions: [
