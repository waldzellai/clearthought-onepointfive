...
import { BaseReasoningNode, BaseReasoningSession, ReasoningPatternType } from './base.js';

/**
 * Ulysses protocol phases (gates)
 */
export type UlyssesPhase = 'reconnaissance' | 'planning' | 'implementation' | 'validation' | 'ship_or_abort';
...
export function advancePhase(
  ...
  const phaseOrder: UlyssesPhase[] = ['reconnaissance', 'planning', 'implementation', 'validation', 'ship_or_abort'];
...
export function detectScopeDrift(session: UlyssesSession): number {
  ...
  for (const change of scopeChanges.map(n => n.scopeChange!)) {
    switch (change.impact) {
      case 'minor': driftScore += 0.2; break;
      case 'moderate': driftScore += 0.5; break;
      case 'major': driftScore += 1.0; break;
    }
  }
...
export function makeFinalDecision(
  ...
  const decision: 'ship' | 'abort' | 'pivot' = 
    session.metrics.confidence >= session.constraints.minConfidence ? 'ship' :
