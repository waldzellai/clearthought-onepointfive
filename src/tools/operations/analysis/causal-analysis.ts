...
export class CausalAnalysisOperation extends BaseOperation {
	...
	async execute(context: OperationContext): Promise<OperationResult> {
		...
		const analysisType = this.getParam(parameters, "analysisType", "structure") as
			| "structure"
			| "intervention"
			| "counterfactual";
	...
	private extractVariables(prompt: string): string[] {
		...
		for (const sentence of sentences) {
			...
			if (causalWords.some((word) => sentence.toLowerCase().includes(word))) {
				...
				const potentialVars = words.filter(
					(word) =>
						word.length > 3 &&
						/^[A-Za-z]/.test(word) &&
	...
	private analyzeStructure(graph: CausalGraph): CausalAnalysisResult {
		...
		const finalEffects = graph.nodes.filter(
			(node) => !graph.edges.some((edge) => edge.from === node),
		);
	...
	private analyzeIntervention(
		...
		notes.push(
			`Intervention on ${intervention.variable} affects ${affectedNodes.length} variables`,
		);
	...
	private performCounterfactualAnalysis(
		...
		if (!intervention) {
			...
			notes.push(
				`Counterfactual: if ${intervention.variable} had not changed, effects would differ`,
			);
		}
