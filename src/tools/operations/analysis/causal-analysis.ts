...
export interface CausalRelationship {
	from: string; // Source variable/cause
	to: string; // Target variable/effect
	weight: number; // Strength of causal relationship (0-1)
	reasoning: string; // AI's explanation for this relationship
	type?: "direct" | "indirect" | "confounded" | "mediated"; // Relationship type
...
export interface CausalAnalysisData {
	...
	causalRelationship: CausalRelationship; // AI provides this structure
	analysisType?: "structure" | "intervention" | "counterfactual" | "mediation";
	isRevision?: boolean;
	revisesEntry?: number;
...
export class CausalAnalysisOperation extends BaseOperation {
	...
	private entryHistory: CausalAnalysisData[] = [];
	private branches: Record<string, CausalAnalysisData[]> = {};
	 ...
	 */
	private validateData(input: unknown): CausalAnalysisData {
		...
		return {
			...
			causalRelationship: {
				...
				type: relationship.type as "direct" | "indirect" | "confounded" | "mediated" | undefined,
				confidence: relationship.confidence as number | undefined,
			},
			analysisType: data.analysisType as
				| "structure"
				| "intervention"
				| "counterfactual"
				| "mediation"
	 ...
	 */
	private formatEntry(data: CausalAnalysisData): string {
		const {
			entryNumber,
			totalEntries,
			entry,
...
└${border}┘`;
	 ...
	 */
	private updateCausalGraph(relationship: CausalRelationship): void {
	 ...
	 */
	private computeGraphStatistics(): Record<string, unknown> {
