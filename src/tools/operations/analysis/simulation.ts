...
export class SimulationOperation extends BaseOperation {
	...
	async execute(context: OperationContext): Promise<OperationResult> {
		...
		const initialState =
			(parameters.initialState as Record<string, number>) || this.parseInitialState(prompt);
	...
	private applySystemDynamicsRule(
		rule: string,
	...
	private runAgentBasedSimulation(
		agentCount: number,
		rules: string[],
		steps: number,
	): SimulationResult {
	...
	private processEvent(
		event: { time: number; type: string; data: any },
		state: Record<string, number>,
	): void {
		switch (event.type) {
			case "increase":
	...
	private runCellularAutomataSimulation(
		gridSize: number,
		rules: string[],
		steps: number,
	): SimulationResult {
