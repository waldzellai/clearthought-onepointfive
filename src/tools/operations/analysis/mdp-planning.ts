...
export class MdpPlanningOperation extends BaseOperation {
	...
	private buildRewardTable(
		rewards: Reward[],
		states: State[],
		actions: Action[],
	): Record<State, Record<Action, number>> {
	...
	private valueIteration(params: MdpParameters) {
		...
		let values: Record<State, number> = Object.fromEntries(
			states.map((s) => [s, params.initialValues?.[s] ?? 0]),
		);
		let policy: Record<State, Action> = Object.fromEntries(
