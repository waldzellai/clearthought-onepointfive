...
/**
 * Mental model journal entry data structure
 */
interface MentalModelData {
	/** Current step in mental model application */
	entry: string;
	/** Current entry number */
	entryNumber: number;
	/** Estimated total entries needed */
	totalEntries: number;
	/** Whether another entry is needed */
	nextEntryNeeded: boolean;
	/** Whether this revises a previous entry */
	isRevision?: boolean;
	/** Entry number being revised */
	revisesEntry?: number;
	/** Entry to branch from for alternative exploration */
	branchFromEntry?: number;
	/** Branch identifier */
	branchId?: string;
	/** Mental model being applied */
	modelName?:
		| "first_principles"
		| "pareto_principle"
		| "inversion"
...
export class MentalModelOperation extends BaseOperation {
	 ...
	 */
	private validateData(input: unknown): MentalModelData {
		...
		if (data.modelName && !validModels.includes(data.modelName as string)) {
			throw new Error(
		...
		return {
			...
			modelName: data.modelName as MentalModelData["modelName"],
...
└${border}┘`;
	...
	async execute(context: OperationContext): Promise<OperationResult> {
		...
		try {
			...
			sessionState.addMentalModel(legacyFormat as any);
