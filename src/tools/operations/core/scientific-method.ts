...
/**
 * PHASE 1-2: Scientific Method Journal Entry Interface
 */
interface ScientificMethodData {
	// Required structured journal fields
	entry: string; // Current step in scientific process
	entryNumber: number;
	totalEntries: number;
...
export class ScientificMethodOperation extends BaseOperation {
	...
	// Logging control
	private disableLogging = (process.env.DISABLE_THOUGHT_LOGGING || "").toLowerCase() === "true";
	...
	private validateData(input: unknown): ScientificMethodData {
		...
		if (
			data.phase &&
			!["observation", "hypothesis", "experiment", "analysis", "conclusion"].includes(
				data.phase as string,
			)
		) {
			throw new Error(
				"Invalid phase: must be one of observation, hypothesis, experiment, analysis, conclusion",
			);
		}
		...
		return {
			...
			phase: data.phase as ScientificMethodData["phase"],
