...
interface MetacognitiveData {
	...
	entry: string; // Metacognitive reflection content
	entryNumber: number;
	totalEntries: number;
	...
	awareness?: string; // Self-awareness of thinking process
	evaluation?: string; // Evaluation of current approach
	confidence?: number; // Confidence level (0-1)
	biasCheck?: string[]; // Identified biases or assumptions
}
...
export class MetacognitiveMonitoringOperation extends BaseOperation {
	...
	private entryHistory: MetacognitiveData[] = [];
	private branches: Record<string, MetacognitiveData[]> = {};
...
8. Only set nextEntryNeeded to false when metacognitive monitoring is complete`,
	 ...
	 */
	private validateData(input: unknown): MetacognitiveData {
		...
		return {
			...
			awareness: data.awareness as string | undefined,
			evaluation: data.evaluation as string | undefined,
			confidence: data.confidence as number | undefined,
			biasCheck: data.biasCheck as string[] | undefined,
		};
	}
	 ...
	 */
	private formatEntry(data: MetacognitiveData): string {
		const {
			entryNumber,
			totalEntries,
			entry,
