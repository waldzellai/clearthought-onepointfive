...
export class DecisionFrameworkOperation extends BaseOperation {
	...
	private generateCriteria(framework: string): any[] {
		...
		const criteriaNames =
			baseMetrics[framework as keyof typeof baseMetrics] || baseMetrics.pros_cons;
	...
	private applyStakeholderFramework(
		alternatives: string[],
		criteria: any[],
		stakeholders: string[],
	): any {
	...
	private calculateConfidence(analysis: any): number {
		...
		const scores = evaluations.map(
			(e: any) => e.score || e.roi || e.overallSupport || e.weightedTotal || 5,
		);

		const mean = scores.reduce((sum: number, score: number) => sum + score, 0) / scores.length;
		const variance =
			scores.reduce((sum: number, score: number) => sum + (score - mean) ** 2, 0) / scores.length;
