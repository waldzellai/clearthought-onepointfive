	...
	async execute(context: OperationContext): Promise<OperationResult> {
		...
		switch (action) {
			case "start": {
				...
				const globalSelection = this.getParam(
					context.parameters,
					"globalSelection",
					{},
				) as PDRSelectionCriteria;
			...
			case "run_pass": {
				...
				const passName = this.getParam(
					context.parameters,
					"passName",
					"scan",
				) as PDRSession["passes"][0]["name"];
			...
			case "select": {
				...
				const criteria = this.getParam(
					context.parameters,
					"criteria",
					session.globalSelection,
				) as PDRSelectionCriteria;
