	...
	try {
		...
		if (args.operation === "code_execution") {
			...
			if (lang !== "python" || !cfg.allowCodeExecution) {
				const preview = await executeClearThoughtOperation(sessionState, args.operation, {
					prompt: args.prompt,
		...
		const result = await executeClearThoughtOperation(sessionState, args.operation, {
			prompt: args.prompt,
			parameters: args.parameters,
		});
	...
	switch (operation) {
		case "sequential_thinking": {
			...
			if (chosenPattern !== "chain" && !(parameters as any).__disablePatternDispatch) {
				...
				if (mappedOp) {
					patternResult = await executeClearThoughtOperation(sessionState, mappedOp, {
						prompt,
		...
		case "creative_thinking": {
			...
			const connections = (parameters.connections as string[]) || [];
			...
			if (insights.length === 0 && ideas.length > 0) {
				...
				const rankedIdeas = ideas
					.map((idea) => {
						...
						const coverage =
							ideaTokens.filter((t) => promptTokens.has(t)).length / ideaTokens.length;
		...
		case "visual_reasoning": {
			...
			return {
				...
				sessionContext: {
					...
					recentOperations: allOperations.slice(-3).map((v) => ({
		...
		case "collaborative_reasoning": {
			...
			const contributions = (parameters.contributions as any[]) || [];
		...
		case "decision_framework": {
			...
			if (analysisType === "expected-utility" && possibleOutcomes.length > 0) {
				...
				options.forEach((opt) => {
					const outcomes = possibleOutcomes.filter(
						(o) => o.option === opt.id || o.option === opt.name,
					);
					expectedValues[opt.id || opt.name] = outcomes.reduce(
						(sum, o) => sum + o.probability * o.value,
						0,
					);
				});
				result.expectedValues = expectedValues;
				const bestOption = Object.entries(expectedValues).reduce(
					(best, [key, val]) => (val > best.value ? { id: key, value: val } : best),
					{ id: "", value: -Infinity },
				);
				...
				const bestOption = Object.entries(scores).reduce(
					(best, [key, val]) => (val > best.value ? { id: key, value: val } : best),
					{ id: "", value: -Infinity },
				);
				result.recommendation = bestOption.id;
		...
		case "structured_argumentation": {
			const argumentData = {
				...
				nextArgumentNeeded: parameters.nextArgumentNeeded,
			};
		...
		case "systems_thinking": {
			...
			const systemsData = {
				...
				iteration: parameters.iteration || 1,
				nextAnalysisNeeded: parameters.nextAnalysisNeeded,
			};
		...
		case "research": {
			...
			const findings = (parameters.findings as ResearchFinding[]) || [];
			const citations = (parameters.citations as ResearchSource[]) || [];
		...
		case "analogical_reasoning": {
			...
			const mappings = (parameters.mappings as AnalogyMapping[]) || [];
			const inferredInsights = (parameters.inferredInsights as string[]) || [];
					...
					switch (mapping.mappingType) {
						case "role":
							inferredInsights.push(
								`${mapping.targetConcept} plays a similar role to ${mapping.sourceConcept}`,
							);
							break;
						case "structure":
							inferredInsights.push(
								`The structure of ${mapping.targetConcept} mirrors ${mapping.sourceConcept}`,
							);
							break;
						case "behavior":
							inferredInsights.push(
								`${mapping.targetConcept} behaves similarly to ${mapping.sourceConcept}`,
							);
							break;
						case "constraint":
							inferredInsights.push(
								`${mapping.targetConcept} has similar constraints as ${mapping.sourceConcept}`,
							);
							break;
					}
				}
		...
		case "statistical_reasoning": {
			...
			if (mode === "summary") {
				...
				const percentile = {
					...
					p50: sorted.length ? sorted[Math.floor(0.5 * (sorted.length - 1))] : 0,
		...
		case "optimization": {
			...
			const variables =
				(parameters.variables as Record<string, { min: number; max: number; step?: number }>) || {};
		...
		case "ethical_analysis": {
			...
			const findings = (parameters.findings as string[]) || [];
			const risks = (parameters.risks as string[]) || [];
			const mitigations = (parameters.mitigations as string[]) || [];
			...
			if (findings.length === 0 && risks.length === 0 && prompt) {
				...
				const frameworkKeywords: Record<
					string,
					{ positive: string[]; negative: string[]; mitigation: string[] }
				> = {
					utilitarian: {
			...
			return await executeClearThoughtOperation(sessionState, "sequential_thinking", {
				...
				parameters: {
					...
					patternParams: {
						...
						explorationConstant: (parameters as any).explorationConstant || Math.SQRT2,
		...
		case "ooda_loop": {
			...
			// Calculate metrics
			node.phaseTimeMs =
				Date.now() - new Date(oodaSession.loopStartTime || oodaSession.createdAt).getTime();
