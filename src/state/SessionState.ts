...
import type { OODASession } from "../types/reasoning-patterns/ooda-loop.js";
import type { PDRSession } from "../types/reasoning-patterns/pdr.js";
import type { UlyssesSession } from "../types/reasoning-patterns/ulysses-protocol.js";
import { type DeploymentMode, PDRKnowledgeGraph } from "./PDRKnowledgeGraph.js";
...
export class SessionState {
		...
		this.unifiedStore = new UnifiedStore(
			config.persistenceEnabled
				? {
						...
						knowledgeGraphFile: config.knowledgeGraphFile,
					}
				: undefined,
		);
	...
	addArgumentation(argument: ArgumentData | SocraticData): void {
		this.touch();
		const isSocratic =
			(argument as SocraticData).question !== undefined ||
			(argument as SocraticData).stage !== undefined;
		if (isSocratic) {
			const id = `socratic-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
			this.unifiedStore.add(id, {
				type: "socratic",
				data: argument as SocraticData,
			});
		} else {
			const id = `argument-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
			this.unifiedStore.add(id, {
				type: "argument",
				data: argument as ArgumentData,
			});
		}
	}
