...
// Cell types matching Srcbook schema
...
export type Cell = TitleCell | MarkdownCell | PackageJsonCell | CodeCell;

export interface SrcbookMetadata {
  language: 'javascript' | 'typescript';
  'tsconfig.json'?: string;
}

export interface ParsedSrcbook {
  metadata: SrcbookMetadata;
  cells: Cell[];
  title: string;
}

export interface SrcbookResource {
  uri: string;
  name: string;
  title?: string;
  description?: string;
...
export function parseSrcbook(contents: string, filename: string): ParsedSrcbook {
  ...
  const titleCell = cells.find(c => c.type === 'title') as TitleCell;
 ...
 */
function extractMetadata(tokens: Token[]): SrcbookMetadata {
 ...
 */
export function encodeSrcbook(srcbook: ParsedSrcbook): string {
  const parts: string[] = [];
